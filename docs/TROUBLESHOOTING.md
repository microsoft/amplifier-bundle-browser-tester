...
# Ensure version consistency if using npx (replace with your installed version)
...
**Solution**:
```bash
# Wait for element to appear (uses default timeout)
...
**Solution**:
```bash
# Ignore HTTPS errors (ONLY for local dev with self-signed certs - disables TLS verification)
...
# The --debug flag works on any command
agent-browser --debug snapshot -ic
```
...
- [Browser Guide](../context/browser-guide.md)
